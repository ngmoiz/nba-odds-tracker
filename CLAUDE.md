...
| 2026-07-15 | **Dette identifiée (revue temps 4) — à corriger avant 1.4/1.6** : `favored_selection()` ignore la direction du spread | Pour un signal piloté par le spread avec h2h plat, la sélection est un tie-break arbitraire → verdict *directionnellement* faux (mauvais côté de ligne enregistré). Cas reproduit : « SIGNAL Miami +6 » alors que la steam va sur Boston -6. **Fix** : dériver l'équipe pressentie du sens du mouvement de spread, pas seulement du h2h. **Priorité haute.** |
